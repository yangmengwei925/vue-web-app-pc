      ...
      <div class="nav">
          ...
          <div class="bot_nav">
            <router-link 
              ...
              :key="item.key" 
              tag="span" 
              @click.native="navClick(item.key)">{{item.name}}</router-link>
          </div>   
      </div>  
  </div>
</div>
...
export default {
  ...
  methods:{
    ...
    loginIn(){
      ...
      //如果为注册 需要多校验一些数据
      if(reShow){
        isObj = [...isObj,dataJson('邮箱',email,'email'),dataJson('年龄',age,'num')];
      }
    ...
    //数据请求
    async requestFunc(){
        const {user,pwd,email,age,reShow,url,$message,$http,$md5,setCookie,$httpSuccess} = this;
